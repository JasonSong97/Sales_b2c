...
아래 문서를 먼저 읽고, User Web의 회사/거래처/제품 화면에 새 딜 count와 연결 딜 목록 API를 반영해줘.
...
Backend 추가 API가 구현된 뒤 User Web에서 회사/거래처/제품 화면에 연결 딜 정보를 표시한다.
...
- 회사/거래처/제품의 기존 생성/수정/메모 동작을 변경하지 않는다.
...
- 거래처 상세에서 연결 딜 목록이 `createdAt DESC` 순서로 보인다.
- 제품 목록에서 `딜 수`가 표시된다.
