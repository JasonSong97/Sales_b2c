...
- `API-SPEC/CONTACT_API.md`: 거래처 도메인 User API 계약
- `API-SPEC/CONTACT_API_DETAIL.md`: 거래처 도메인 User API 요청값, 응답값, 내부 비즈니스 로직, DB, 에러, FE/BE 처리 기준
...
- 거래처 기본 기능에는 휴지통과 soft delete를 넣지 않는다.
- `contactMemo`는 `Contact` 테이블 컬럼이 아니다.
- 거래처 생성의 `contactMemo`는 값이 있을 때만 `ContactMemoLog` 첫 데이터로 저장한다.
- 거래처 생성의 `contactMemo`는 `memoType`을 `초기 메모`로 저장한다.
- 독립적인 거래처 일반 메모 로그 생성은 `memoType`, `memo`를 받는다.
- 독립적인 거래처 일반 메모 로그 수정은 `memoType`, `memo` 중 최소 1개를 받는다.
- 독립적인 거래처 개인 비밀 메모 로그 생성/수정은 `memo`만 받는다.
- 거래처 개인 비밀 메모 원문은 DB에 평문으로 저장하지 않는다.
