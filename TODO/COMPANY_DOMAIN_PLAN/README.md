...
관리자 페이지, 휴지통, soft delete, 딜 수 계산은 현재 범위에서 제외한다. 회사 목록의 거래처 수(`contactCount`)와 회사 단건 화면 보조 영역의 연결 Contact 목록은 추가 유지보수 범위에서 Backend API가 구현되어 FE 표시 범위에 포함한다.
...
- 회사 목록 item에는 `contactCount`가 있고, FE는 이를 거래처 수로 표시한다.
