...
- 이 기본 goal에서는 회사 목록에 거래처 수와 딜 수를 추가하지 않는다.
- 이 기본 goal에서는 회사 단건 응답에 거래처 수를 병합하거나 딜 수를 추가하지 않는다.
...
이 기본 goal 완료 이후 `TODO/ADDITIONAL_WORK_PLAN`에서 아래 Backend API가 추가 구현됐다.

- `GET /api/companies` 목록 item에 `contactCount` 추가
- `GET /api/companies/:companyId/contacts`
- `GET /api/companies/export/xlsx`

따라서 현재 FE 작업자는 회사 목록의 거래처 수는 `contactCount`로 표시하고, 회사 단건 화면의 연결 Contact 목록은 별도 API로 조회한다. 회사 단건 응답 자체에 거래처 수와 딜 수를 병합하는 변경은 여전히 범위 밖이다.
...
- 회사 분야/지역 수정
- 딜 수 계산
- 회사 단건 응답에 거래처 수 또는 딜 수를 병합하는 변경
