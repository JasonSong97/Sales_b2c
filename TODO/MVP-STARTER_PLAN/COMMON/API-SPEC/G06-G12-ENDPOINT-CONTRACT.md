...
| 회사 삭제 API | `DeleteCompany` | `DeleteCompanyRequest`: `companyId` path 필수 | 소유권을 확인한다. 이미 삭제된 경우 `409 DeletedResource`를 반환한다. 삭제되지 않은 회사는 `deletedAt`을 기록한다. | `DeleteCompanyResponse`: `id`, `deletedAt` | `Company.deletedAt` update. transaction 없음. | `CompanyNotFound` 404, `OwnershipViolation` 403, `DeletedResource` 409 |
...
| 거래처 삭제 API | `DeleteContact` | `DeleteContactRequest`: `contactId` path 필수 | 소유권을 확인한다. 이미 삭제된 경우 `409 DeletedResource`를 반환한다. 삭제되지 않은 거래처는 `deletedAt`을 기록한다. 연결 딜은 자동 삭제하지 않는다. | `DeleteContactResponse`: `id`, `deletedAt` | `Contact.deletedAt` update. transaction 없음. | `ContactNotFound` 404, `OwnershipViolation` 403, `DeletedResource` 409 |
...
| 제품 삭제 API | `DeleteProduct` | `DeleteProductRequest`: `productId` path 필수 | 소유권을 확인한다. 이미 삭제된 경우 `409 DeletedResource`를 반환한다. 삭제되지 않은 제품은 `deletedAt`을 기록한다. 기존 딜 연결은 유지하되 삭제 상태 제품으로 표시한다. | `DeleteProductResponse`: `id`, `deletedAt` | `Product.deletedAt` update. transaction 없음. | `ProductNotFound` 404, `OwnershipViolation` 403, `DeletedResource` 409 |
...
| 딜 삭제 API | `DeleteDeal` | `DeleteDealRequest`: `dealId` path 필수 | 딜 소유권을 확인한다. 이미 삭제된 경우 `409 DeletedResource`를 반환한다. 삭제되지 않은 딜은 `deletedAt`을 기록한다. 관련 활동과 일정은 자동 삭제하지 않는다. | `DeleteDealResponse`: `id`, `deletedAt` | `Deal.deletedAt` update. transaction 없음. | `DealNotFound` 404, `OwnershipViolation` 403, `DeletedResource` 409 |
