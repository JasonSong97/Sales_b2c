...
이 폴더는 `한손에 영업 / onehand.sales` MVP 구현을 시작하기 위한 실행 작업 목록을 정리한다.

`AGENT` 문서가 제품과 아키텍처의 정본이라면, `TODO` 문서는 실제 구현 순서와 작업 단위를 관리하기 위한 실행 문서다. 구현 중 새로운 결정이 생기면 먼저 `AGENT/DECISIONS`에 확정 내용을 남기고, 그 결과를 이 TODO 문서에 반영한다.
