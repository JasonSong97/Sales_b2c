...
- 거래처 일반 메모 로그 수정 API는 `memoType`, `memo` 중 최소 1개를 수정할 수 있다.
- 거래처 개인 비밀 메모는 DB에 평문으로 저장하지 않는다.
...
- 거래처 직급 생성/삭제 API
- 거래처 부서 생성/삭제 API
- 거래처 일반 메모 로그 단건 생성 API
- 거래처 일반 메모 로그 10개씩 무한스크롤 조회 API
- 거래처 일반 메모 로그 단건 수정 API
- 거래처 개인 비밀 메모 로그 단건 생성 API
- 거래처 개인 비밀 메모 로그 10개씩 무한스크롤 조회 API
- 거래처 개인 비밀 메모 로그 단건 수정 API
- `contactMemo`가 있는 거래처 생성 요청의 transaction 처리
- 거래처 API 계약의 observability event key, request id, redaction 기준 유지
- 거래처 직급/부서 삭제 전 매핑 여부 검사
- 개인 비밀 메모 암호화/복호화 처리
...
- FE 화면 구현
- 관리자 API 추가
- 거래처 휴지통 또는 soft delete 추가
- 거래처 삭제 API 추가
- 거래처 직급/부서 수정 API 추가
- 회사 없이 거래처 저장 허용
- 명함 OCR 저장 연동
- 거래처 상세에 딜 수, 제품 수, 일정 수, 회의록 수 추가
...
- 거래처 일반 메모 로그 생성 UI
- 거래처 일반 메모 로그 10개씩 무한스크롤 조회 UI
- 거래처 일반 메모 로그 수정 UI
- 거래처 개인 비밀 메모 로그 생성 UI
- 거래처 개인 비밀 메모 로그 10개씩 무한스크롤 조회 UI
- 거래처 개인 비밀 메모 로그 수정 UI
