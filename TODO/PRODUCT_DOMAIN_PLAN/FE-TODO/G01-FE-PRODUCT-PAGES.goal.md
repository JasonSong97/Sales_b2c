...
- [ ] 제품 상세 화면이 계약 응답값을 표시한다.
- [ ] 제품 수정 화면이 계약 요청값만 전송한다.
- [ ] 일반 메모 생성/조회/수정이 동작한다.
- [ ] 일반 메모 수정에서 `memoType`과 `memo` 중 최소 1개를 수정할 수 있다.
- [ ] 개인 비밀 메모 생성/조회/수정이 동작한다.
- [ ] 본문 없는 `201 Created`, `204 No Content` 응답을 정상 처리한다.
- [ ] 오래된 필드명과 삭제/복구/연결/로그 UI가 제거되어 있다.
