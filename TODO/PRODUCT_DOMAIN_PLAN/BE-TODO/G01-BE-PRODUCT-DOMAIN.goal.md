...
`TODO/PRODUCT_DOMAIN_PLAN/COMMON/API-SPEC/PRODUCT_API_DETAIL.md`를 기준으로 아래 API를 구현한다.
...
- [ ] 모든 Product API가 구현되어 있다.
- [ ] 제품 목록 검색은 `productName`만 대상으로 한다.
- [ ] 제품 생성 + 선택적 초기 메모 생성이 한 트랜잭션으로 처리된다.
- [ ] 개인 비밀 메모는 암호화 저장되고 응답에서만 복호화된다.
- [ ] 일반 메모 수정에서 `memoType`과 `memo` 중 최소 1개를 수정할 수 있다.
- [ ] 개인 비밀 메모 수정 API가 존재한다.
- [ ] 카테고리/상태 삭제 시 사용 중이면 `409 Conflict`를 반환한다.
- [ ] API별 주석, 기능 주석, 클래스/인터페이스 역할 주석이 반영되어 있다.
- [ ] 관측성 로그가 민감정보 없이 작성되어 있다.
- [ ] API 계약 문서가 실제 구현과 일치한다.
- [ ] AGENT/BE 문서가 Product 구현 상태와 일치한다.
