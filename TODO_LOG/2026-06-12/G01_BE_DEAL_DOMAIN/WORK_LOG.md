...
- Prisma `Deal`, `DealFollowingActionLog`, `DealMemoLog` 모델과 migration 추가
...
- Prisma schema에 Deal 관련 모델 3개와 User/Company/Contact/Product relation을 추가했다.
...
- Deal 생성 시 Deal과 최초 `DealFollowingActionLog` 생성이 같은 repository transaction 안에서 실행되도록 구현했다.
...
- `pnpm run typecheck`
- `pnpm test -- --runInBand` -> 6 suites, 15 tests passed
- `pnpm run lint`
- `pnpm run build`
- `GET http://localhost:3000/api/health` -> 200
- `GET http://localhost:3000/api/deals/stage-counts` without auth -> 401
- 임시 clean schema `codex_deal_verify_*`에 현재 repo migration 4개 적용 성공
- 임시 clean schema에서 `Deal`, `DealFollowingActionLog`, `DealMemoLog` 테이블과 `Deal.expectedEndDate` date 컬럼 확인
...
- 현재 repo migration 4개는 local DB에 적용되어 있지 않은 상태로 표시된다.
- local DB에는 예전 MVP `Deal` 테이블만 존재하며, 컬럼은 `title`, `amount`, `stage`, `likelihoodStatus`, `nextActionTitle` 등으로 현재 계약의 `dealName`, `dealCost`, `dealStatus`, `DealFollowingActionLog`, `DealMemoLog` 구조와 다르다.
