...
import {
  contactLogFormSchema,
  toCreateContactLogInput,
  toDateTimeLocalValue,
  toUpdateContactLogInput,
  type ContactLogFormValues,
...
export function ContactLogSection({
  ...
  const createLogMutation = useCreateContactLogMutation();
  const updateLogMutation = useUpdateContactLogMutation();
  const deleteLogMutation = useDeleteContactLogMutation(contactId);
  const editingLog = useMemo(
    () => logs.find((log) => log.id === editingLogId) ?? null,
    [editingLogId, logs]
  );
  const createForm = useForm<ContactLogFormValues>({
    resolver: zodResolver(contactLogFormSchema),
    defaultValues: getEmptyLogValues(),
  });
  const editForm = useForm<ContactLogFormValues>({
    resolver: zodResolver(contactLogFormSchema),
    defaultValues: getEmptyLogValues(),
  });
  ...
  const onCreate = createForm.handleSubmit(async (values) => {
    await createLogMutation.mutateAsync(toCreateContactLogInput(contactId, values));
    createForm.reset(getEmptyLogValues());
    onChanged("거래처 로그가 추가되었습니다.");
  });
  ...
  const onUpdate = editForm.handleSubmit(async (values) => {
    ...
    await updateLogMutation.mutateAsync(
      toUpdateContactLogInput(contactId, editingLog.id, values)
    );
    setEditingLogId(null);
    onChanged("거래처 로그가 수정되었습니다.");
  });

  const onDelete = async (logId: string) => {
    await deleteLogMutation.mutateAsync(logId);
    onChanged("거래처 로그가 삭제되었습니다.");
  };
      ...
      <form className="grid gap-3 rounded-lg border bg-white p-4" onSubmit={onCreate}>
        <div className="grid gap-3 md:grid-cols-[180px_1fr]">
          <div className="grid gap-2">
            <label className="text-sm font-medium" htmlFor="contact-log-date">
              기록 시간
            </label>
            <input
          ...
          <div className="grid gap-2">
            <label className="text-sm font-medium" htmlFor="contact-log-title">
              제목
            </label>
            <input
              aria-describedby={
                createForm.formState.errors.title
                  ? "contact-log-title-error"
                  : undefined
              }
              aria-invalid={Boolean(createForm.formState.errors.title)}
              className="h-10 rounded-md border px-3 text-sm outline-none focus:ring-2 focus:ring-ring"
              id="contact-log-title"
              {...createForm.register("title")}
            />
            {createForm.formState.errors.title ? (
              <p className="text-xs text-destructive" id="contact-log-title-error">
...
function getEmptyLogValues(): ContactLogFormValues {
  return {
