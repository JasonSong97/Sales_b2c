...
import {
  useCreateProductLogMutation,
  useDeleteProductLogMutation,
  useUpdateProductLogMutation,
} from "@/features/product/hooks/use-product-mutations";
import {
  productLogFormSchema,
  toCreateProductLogInput,
  toDateTimeLocalValue,
  toUpdateProductLogInput,
  type ProductLogFormValues,
} from "@/features/product/schemas/product-schema";
import type { ProductLog } from "@/features/product/types/product";
import { getApiErrorMessage } from "@/lib/api-client";

type ProductLogSectionProps = {
...
export function ProductLogSection({
  productId,
  logs,
  isLoading,
  error,
  onRetry,
  onChanged,
}: ProductLogSectionProps) {
  const [editingLogId, setEditingLogId] = useState<string | null>(null);
  const createLogMutation = useCreateProductLogMutation();
  const updateLogMutation = useUpdateProductLogMutation();
  const deleteLogMutation = useDeleteProductLogMutation(productId);
  const editingLog = useMemo(
    () => logs.find((log) => log.id === editingLogId) ?? null,
    [editingLogId, logs]
  );
  const createForm = useForm<ProductLogFormValues>({
    resolver: zodResolver(productLogFormSchema),
    defaultValues: getEmptyLogValues(),
  });
  const editForm = useForm<ProductLogFormValues>({
    resolver: zodResolver(productLogFormSchema),
    defaultValues: getEmptyLogValues(),
  });
  ...
  const onCreate = createForm.handleSubmit(async (values) => {
    await createLogMutation.mutateAsync(toCreateProductLogInput(productId, values));
  ...
  const onUpdate = editForm.handleSubmit(async (values) => {
    ...
    await updateLogMutation.mutateAsync(
      toUpdateProductLogInput(productId, editingLog.id, values)
    );
  ...
  const onDelete = async (logId: string) => {
    ...
    onChanged("제품 로그가 삭제되었습니다.");
  };

  return (
    <section className="grid gap-4">
      <div>
        <h2 className="text-lg font-semibold">Log 기록</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          제안, 납품, 가격 변경, 경쟁 상황을 기록합니다.
        </p>
      </div>
      ...
      <form className="grid gap-3 rounded-lg border bg-white p-4" onSubmit={onCreate}>
        <div className="grid gap-3 md:grid-cols-[180px_1fr]">
          <div className="grid gap-2">
            <label className="text-sm font-medium" htmlFor="product-log-date">
              기록 시간
            </label>
            <input
          ...
          <div className="grid gap-2">
            <label className="text-sm font-medium" htmlFor="product-log-title">
              제목
            </label>
            <input
              aria-describedby={
                createForm.formState.errors.title
                  ? "product-log-title-error"
                  : undefined
              }
              aria-invalid={Boolean(createForm.formState.errors.title)}
              className="h-10 rounded-md border px-3 text-sm outline-none focus:ring-2 focus:ring-ring"
              id="product-log-title"
              {...createForm.register("title")}
            />
            {createForm.formState.errors.title ? (
              <p className="text-xs text-destructive" id="product-log-title-error">
...
function getEmptyLogValues(): ProductLogFormValues {
  return {
