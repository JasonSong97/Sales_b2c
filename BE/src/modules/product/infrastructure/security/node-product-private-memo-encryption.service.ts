...
// 역할 : NodeProductPrivateMemoEncryptionService 제품 개인 비밀 메모 암호화 기능을 제공합니다.
@Injectable()
export class NodeProductPrivateMemoEncryptionService
  implements ProductPrivateMemoEncryptionPort
{
  // 기능 : 제품 개인 비밀 메모 암호화에 필요한 설정 서비스를 주입받습니다.
  constructor(private readonly configService: ConfigService) {}

  // 기능 : 제품 개인 비밀 메모 평문을 AES-256-GCM 암호문으로 변환합니다.
  encrypt(plaintext: string): EncryptedProductPrivateMemo {
    try {
      ...
      throw new ProductPrivateMemoEncryptFailedError();
    }
  }

  // 기능 : AES-256-GCM 암호문을 제품 개인 비밀 메모 평문으로 복호화합니다.
  decrypt(ciphertext: string, keyVersion: string): string {
    try {
      ...
      throw new ProductPrivateMemoDecryptFailedError();
    }
  }
  ...
  private getKey(): Buffer {
    const secret =
      this.configService
        .get<string>("PRODUCT_PRIVATE_MEMO_ENCRYPTION_KEY")
        ?.trim() ||
      this.configService.get<string>("ENCRYPTION_MASTER_KEY")?.trim();

    if (!secret) {
      throw new Error("Missing product private memo encryption key");
    }

    return createHash("sha256").update(secret).digest();
  }

  // 기능 : 저장할 제품 개인 비밀 메모 암호화 key version을 조회합니다.
  private getKeyVersion(): string {
    return (
      this.configService
        .get<string>("PRODUCT_PRIVATE_MEMO_ENCRYPTION_KEY_VERSION")
        ?.trim() ||
