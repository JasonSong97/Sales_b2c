...
## 14. 설계 철학 — Admin 도구 엔지니어 10년차의 관점

> Admin 웹은 사용자 앱과 결정적으로 다른 책임을 진다. 아래 4가지가 무너지면 사고가 난다.

### 14.1 "Admin 도구는 부드러워야 하는 게 아니라 정확해야 한다"
- User 앱은 UX·속도·심미성이 가치. Admin은 **정확성·추적성·안전성**이 가치.
- 잘못 누른 버튼 하나로 1000명 사용자의 데이터를 망칠 수 있는 환경이다.
- 화려한 애니메이션·트랜지션 < 명확한 확인 다이얼로그 + 변경 전후 비교.
- → AlertDialog + 사유 입력이 필수인 진짜 이유. "한 번 더 묻는 비용 < 잘못 누른 비용".

### 14.2 "모든 액션은 누가·언제·왜를 남긴다 (Audit Everything)"
- Admin 액션은 법적·내부 분쟁의 증거가 된다. "관리자가 임의로 데이터 바꿨다"는 비난을 받을 때 로그만이 방어 수단.
- DB 변경 + 로그는 **반드시 같은 트랜잭션** — 로그만 빠지면 책임 추궁 불가.
- 사유(reason) 입력을 강제하는 액션 목록을 문서화 (계정 정지, 데이터 강제 수정 등).
- → 감사 로그는 "있으면 좋은 것"이 아니라 **Admin의 합법성 자체**.

### 14.3 "사용자가 적고 데이터가 크다"
- User 앱: 사용자 1만 명 × 데이터 50개 = 부드러운 응답 우선
- Admin: 관리자 5명 × 데이터 50만 행 = 데이터 그리드/페이지/필터/검색이 핵심
- → TanStack Table + 서버 페이지네이션이 디폴트인 진짜 이유. 클라이언트 페이지네이션은 1만 건 넘으면 죽는다.
- 차트도 마찬가지: 실시간 X, 5분 캐시 + 수동 새로고침이 합리적.

### 14.4 "User 코드와 의도적으로 분리한다 (Two Codebases, One Backend)"
- "공통 컴포넌트 재사용" 유혹을 거절한다. Admin이 User UX에 종속되거나, User가 Admin 보안에 영향 받으면 둘 다 약해진다.
- 같은 도메인(거래처) 표시도 Admin은 row 단위 작업, User는 카드 형태 — **UX 요구 자체가 다르다**.
- 백엔드만 공유, 클라이언트는 별도 레포 + 별도 디자인.
- → "Admin 분리 시 modules/admin/ 폴더만 떼면 됨" 약속의 클라이언트 측 거울. 백엔드만 분리 가능해서는 의미 없음.
...
7. "1만 건 넘는 리스트는 반드시 서버 페이지네이션"
8. "Admin 액션의 DB 변경과 감사 로그는 같은 트랜잭션"
9. "감사 사유 입력이 강제되는 액션 목록을 코드 상수로 관리"
