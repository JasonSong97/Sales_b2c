...
## 15. 설계 철학 — 프론트엔드 엔지니어 10년차의 관점

> 이 문서의 모든 규칙은 아래 4가지 마음가짐에서 파생된다. 규칙이 충돌할 때 이 철학으로 돌아온다.

### 15.1 "상태의 출처는 단 하나여야 한다 (Single Source of Truth)"
- 서버 상태를 Redux/Zustand에 복사하는 순간 동기화 지옥이 시작된다.
- 서버 상태 = TanStack Query 캐시, 클라이언트 상태 = useState/Zustand, URL 상태 = searchParams. **셋이 섞이면 안 된다.**
- "이 데이터는 어디가 진실인가?"가 1초 안에 답이 안 나오면 설계 잘못된 것.
- → useEffect+fetch가 금지인 진짜 이유. 그건 캐시·동기화·재시도·invalidate를 매번 새로 짜는 일.

### 15.2 "재사용보다 삭제 가능성"
- "나중에 재사용될 수 있으니" 추상화하면 보통 재사용 안 되고 삭제만 어려워진다.
- **세 번째 사용처가 생길 때 비로소 추출**한다 (Rule of Three).
- Feature 폴더 안에 두는 것이 기본, `shared/`로 올리는 건 입증된 후.
- → 한 컴포넌트 150줄 넘으면 분리하는 진짜 이유: 큰 컴포넌트는 지우기 어렵다.

### 15.3 "보이지 않는 사용자 — 키보드와 스크린리더"
- 영업사원이 운전 후 차에서 한 손으로 쓸 수도 있고, 시력이 약할 수도 있다.
- `<div onClick>` 한 줄을 `<button>`으로 바꾸는 데 1초, 사용자 확보 비용은 그 1초보다 크다.
- 폼 라벨, 키보드 네비, 색상 대비 — 출시 후 추가하면 회귀가 많아 사실상 불가능.
- → a11y는 "여유 있을 때 추가하는 것"이 아니라 **컴포넌트 작성 시점에 들어가야** 비용이 0에 가까움.

### 15.4 "성능은 측정 후 최적화"
- React 19부터 컴파일러가 메모이제이션을 알아서 처리한다. `useMemo` 남발은 코드만 더럽힐 뿐.
- "느릴 것 같다"는 직감으로 최적화하면 결국 다른 곳에서 병목이 생긴다.
- 측정 도구: React DevTools Profiler, Lighthouse, Sentry Performance. **숫자가 있어야 최적화 시작.**
- → "기본은 안 함"이 진짜 의도. 가상 스크롤도 1000개 넘는 데이터가 실제로 들어왔을 때만.
...
6. "추상화는 세 번째 사용처가 생길 때만 — Rule of Three"
7. "a11y는 작성 시점에 — `<button>`, `<label htmlFor>`, 키보드 탭 순서"
8. "성능 최적화는 측정 후. `useMemo`/`useCallback` 기본 사용 금지"
